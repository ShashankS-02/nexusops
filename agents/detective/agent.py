...
from nexusops.llm import get_llm, get_embedding_model
from db.qdrant_store import search_similar_incidents

DETECTIVE_PROMPT = ChatPromptTemplate.from_messages([
    ("system", """You are the Detective Agent for NexusOps, an AI Site Reliability Engineer.
...
def detective_node(state: IncidentState) -> dict:
    ...
    try:
        ...
        ) if similar else "No past incidents in knowledge base yet."
